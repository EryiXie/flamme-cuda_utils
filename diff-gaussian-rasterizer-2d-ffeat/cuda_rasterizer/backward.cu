...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3 *means, glm::vec3 campos, const float *shs, const bool *clamped, const glm::vec3 *dL_dcolor, glm::vec3 *dL_dmeans, glm::vec3 *dL_dshs,  float *dL_dtau)
{
	...
	// TODO: Gradients colors to camera pose
								 ...
								 float *dL_dtau)
{
	...
	// TODO: Gradients of loss w.r.t. camera pose
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
	renderCUDA(
		...
		const float *__restrict__ ffeature, // foundation features
		const float *__restrict__ final_Ts,
		const uint32_t *__restrict__ n_contrib,
		const float *__restrict__ dL_dpixels,
		const float *__restrict__ dL_dpixels_features, // foundation feature gradient
	...
	)
{
	...
	float accum_rec[C] = {0};
	float accum_ffeature_rec[NUM_feature_CHANNELS] = { 0 };  // foundation feature gradient
	float dL_dpixel[C]; 
	float dL_dffeaturepixel[NUM_feature_CHANNELS];  // foundation feature gradient
	if (inside)
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		for (int i = 0; i < NUM_feature_CHANNELS; i++) 
			dL_dffeaturepixel[i] = dL_dpixels_features[i * H * W + pix_id]; // foundation feature gradient

	float last_alpha = 0;
	float last_color[C] = {0};
	float last_ffeature[NUM_feature_CHANNELS] = { 0 }; // foundation feature gradient
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float dchannel_dffeature = alpha * T;  // foundation feature gradient
			...
			for (int ch = 0; ch < NUM_feature_CHANNELS; ch++)  // foundation feature gradient
			{
				const float f = collected_foundation_feature[ch * BLOCK_SIZE + j];
				// Update last semantic feature (to be used in the next iteration)
				accum_ffeature_rec[ch] = last_alpha * last_ffeature[ch] + (1.f - last_alpha) * accum_ffeature_rec[ch];
				last_ffeature[ch] = f;

				const float dL_dffeaturechannel = dL_dffeaturepixel[ch];
				// Update the gradients w.r.t. semnatic feature of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dffeature[global_id * NUM_feature_CHANNELS + ch]), dchannel_dffeature * dL_dffeaturechannel); 
			}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < C; i++)
				collected_colors[i * BLOCK_SIZE + block.thread_rank()] = colors[coll_id * C + i];
			for (int i = 0; i < NUM_feature_CHANNELS; i++)
				collected_foundation_feature[i * BLOCK_SIZE + block.thread_rank()] = ffeature[coll_id * NUM_feature_CHANNELS + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < NUM_feature_CHANNELS; ch++)  // foundation feature gradient
			{
				...
				const float dL_dffeaturechannel = dL_dffeaturepixel[ch];
				dL_dalpha += (f - accum_ffeature_rec[ch]) * dL_dffeaturechannel;
				// Update the gradients w.r.t. semnatic feature of the Gaussian. 
				// Atomic, since this pixel is just one of potentially
				// many that were affected by this Gaussian.
				atomicAdd(&(dL_dffeature[global_id * NUM_feature_CHANNELS + ch]), dchannel_dcolor * dL_dffeaturechannel); 
			}
...
void BACKWARD::render(
	...
	const float *colors,
	const float *ffeature, // foundation feature
	const float *final_Ts,
	const uint32_t *n_contrib,
	const float *dL_dpixels,
	const float *dL_dpix_ffeatures, // foundation feature gradient
	...
	float *collected_foundation_feature)
{
	renderCUDA<NUM_CHANNELS, NUM_feature_CHANNELS><<<grid, block>>>(
		...
		conic_opacity,
		colors,
		ffeature, // foundation feature
		final_Ts,
		n_contrib,
		dL_dpixels,
		dL_dpix_ffeatures, // foundation feature gradient
		dL_dpixel_depths,
		hit_image,
		hit_normal_c,
		hit_point_c,
	...
	float *collected_foundation_feature)
{
	...
	renderCUDA_flat<NUM_CHANNELS, NUM_feature_CHANNELS><<<grid, block>>>(
		tile_indices,
		tile_width,
		ranges,
		point_list,
