...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
	renderCUDA(
		...
		float *__restrict__ out_color,
		float *out_ffeature_map, // foundation feature
		float *__restrict__ out_depth,
		...
		int * __restrict__ n_touched)
{
	...
	float SF[feature_CHANNELS] = { 0 }; // foundatino feature /***/
		...
		for (int j = 0; !done && render && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (test_T >= 0.0001f)
			{
				...
				for (int ch = 0; ch < feature_CHANNELS; ch++){
					SF[ch] += ffeature[collected_id[j] * feature_CHANNELS + ch] * alpha * T; 
				} // foundation feature /***/
			...
			// TODO: Added n_touched computation here: is the collected_id[j] the correct index?
	...
	if (inside && render)
	{
		...
		for (int ch = 0; ch < feature_CHANNELS; ch++)                 
			out_ffeature_map[ch * H * W + pix_id] = SF[ch] + T * bg_color[0]; /***TODO:***/ 
...
__global__ void __launch_bounds__(BLOCK_X *BLOCK_Y)
	renderCUDA_withMask(
		...
		const float2 *__restrict__ points_xy_image,
		const float *__restrict__ features,
		const float *__restrict__ ffeature, // foundation feature
		const float *__restrict__ depths,
		const glm::vec3 *__restrict__ scales,
		const glm::vec4 *__restrict__ rotations,
		const float4 *__restrict__ conic_opacity,
		...
		float3 *__restrict__ hit_point_c,
		float *__restrict__ out_color,
		float *__restrict__ out_ffeature_map, // foundation feature
		float *__restrict__ out_depth,
		int *__restrict__ out_hit_depth,
		int *__restrict__ out_hit_color,
		float *__restrict__ out_hit_color_weight,
		...
		int * __restrict__ n_touched)
{
	...
	float SF[feature_CHANNELS] = { 0 }; // foundation feature /***/
	// Iterate over batches until all done or range is complete
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		// End if entire block votes that it is done rasterizing
	...
	int* n_touched)
{
	...
	renderCUDA_withMask<NUM_CHANNELS, NUM_feature_CHANNELS><<<grid, block>>>(
		tile_indices,
		tile_width,
		ranges,
		point_list,
