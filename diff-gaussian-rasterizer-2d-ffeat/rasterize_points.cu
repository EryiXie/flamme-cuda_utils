...
RasterizeGaussiansCUDA(
	...
	const torch::Tensor& ffeature, // foundation feature
	...
	const bool debug)
{
	...
	torch::Tensor out_ffeature_map = torch::full({NUM_feature_CHANNELS, H, W}, 0.0, float_opts); 
	...
	if (P != 0)
	{
		...
		rendered = CudaRasterizer::Rasterizer::forward(
			...
			ffeature.contiguous().data<float>(), // foundation feature
			...
			out_ffeature_map.contiguous().data<float>(), // foundation feature
	...
	return std::make_tuple(rendered, tile_num, out_color, out_depth,
						   ...
						   out_ffeature_map, // foundation feature
...
RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_ffeatures, // foundation feature gradient
	...
	const bool debug)
{
	...
	torch::Tensor dL_dffeature = torch::zeros({P, ffeature.size(1), NUM_feature_CHANNELS}, means3D.options()); // foundation feature gradient /********hard-coded********/
	...
	if (P != 0)
	{
		CudaRasterizer::Rasterizer::backward(P, degree, M, R,
											 ...
											 dL_dout_ffeatures.contiguous().data<float>(), // foundation feature gradient
											 ...
											 dL_dffeature.contiguous().data<float>(), // foundation feature gradient
	...
	return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dffeature, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dtau);
}
